...
- 원인: `duplicateNameKey` strip 패턴 `[\s\-_/()（）]`이 `% ! , . &` 등 기호를 남김 — census `normalize.py`도 **동일 패턴**이라 둘이 어긋나지 않음(정규화 버그 아님).
- **결정(2026-06-26): 큐 축소 안 함, 현행 유지.** 기호(`. % &` 등)는 수량·농도·구성을 담아, 일괄 strip 시 과병합(`1.5L`↔`15L`, `과즙100%`↔`100`, `사과&배`↔`사과배`) 위험. 실패 비용이 비대칭 — 과분할(충돌로 남김)은 무해(양쪽 검색 가능, 큐 항목 1개), 과병합은 유해·조용함(패자 숨겨져 잘못된 영양정보 노출, 되돌리기 어려움). 따라서 안전한 방향(과분할)을 택하고 충돌은 검토 큐로 둔다(설계 §9 자동병합 금지와 일치).

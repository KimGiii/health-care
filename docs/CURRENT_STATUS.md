...
- 2026-04-22 기준, 인증/사용자 컨트롤러 테스트, 식단 검색 품질 보정, 진행 사진 iOS 화면, 목표 단위 정규화, 홈·기록·마이 화면 개편을 반영했다.
- Phase 1은 `AuthControllerTest`, `UserControllerTest` 추가와 Bearer 헤더 기반 사용자 식별 정리로 92%까지 상향했다.
...
- [~] 컨트롤러/보안 테스트: Auth/User/Goal은 MockMvc 단위 테스트가 있으나 JWT 필터/보안 체인 검증이 더 필요
- [~] iOS 진행 사진: 촬영 시점 선택, 삭제/비교 UX, 썸네일 상태 fallback 개선 필요

### 아직 미구현 또는 미완성

- [ ] FCM 기반 인사이트/알림 흐름
- [ ] JWT 필터/보안 체인 중심 통합 테스트
...
1. **JWT 필터/보안 체인 통합 테스트** — 인증 누락, 만료 토큰, 권한 경계 검증
